...
public class BitList {
    ...
    public BitList( String s ) {
      ...
      for (int j = 0; j < s.length (); j++) {
        num [j] = Integer.parseInt(String.valueOf(s.charAt(j)));
      }
        for (int i = 0; i < num.length; i++) {
          if ( (  num[i] != ZERO ) && ( num[i] != ONE ) ) {
              throw new IllegalArgumentException( Integer.toString( num[i] ));
          }
          first = new Node (num[i], first);
        }
    }
